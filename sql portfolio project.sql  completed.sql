CREATE SCHEMA Hospital_management_db;
...
CREATE TABLE doctors (
    ...
    hospital_id INT NOT NULL,
    email VARCHAR(255) UNIQUE, -- Ensuring unique email
...
CREATE TABLE rooms (
    ...
    room_no VARCHAR(10) NOT NULL, -- Use VARCHAR for room numbers like "A101"
...
INSERT INTO hospitals (hospital_id, hospital_name, address, phone)
VALUES
...
INSERT INTO patients (patient_id, patients_name, dob, address, phone)
VALUES
...
INSERT INTO departments (department_id, hospital_id, dep_name)
VALUES
(1, 1, 'Cardiology'),
(2, 1, 'Radiology'),
(3, 1, 'Oncology'),
...
INSERT INTO medications (medication_id, medication_name, description)
VALUES 
...
SELECT
    d.doctor_id,
    d.doc_name,
    d.hospital_id AS FK_Hospital_ID,
    h.hospital_name AS Linked_Hospital
FROM
    doctors d
LEFT JOIN
    hospitals h ON d.hospital_id = h.hospital_id
WHERE
    h.hospital_id IS NULL; -- CRITICAL: Filters for unmatched records
...
where month(appointment_date) = 8;


select medication_name, description
from medications
...
group by p.patient_id,p.patients_name
HAVING COUNT(pr.medication_id) > 1;


WITH DoctorAppointmentCounts AS (
    SELECT
        d.doctor_id,
        d.doc_name,
        d.hospital_id,
    ...
    JOIN
        hospitals h ON d.hospital_id = h.hospital_id
    GROUP BY
        d.doctor_id, d.doc_name, d.hospital_id, h.hospital_name
)
SELECT
    Doc_Name,
    Hospital_Name,
...
WITH EmergencyPatients AS (
    ...
    WHERE
        a.reason LIKE '%emergency%'
        OR a.reason LIKE '%critical%'
)
...
SELECT
    ...
    Age,
    Age_Group,
    -- Step 2: Rank patients by age within each age group
    RANK() OVER (
...
WITH CardioConsultations AS (
    ...
    WHERE d.specialty = 'Cardiology'
      AND a.reason LIKE '%Consultation%'
    GROUP BY Patient_Age_Group
)
SELECT *,
       RANK() OVER (ORDER BY Total_Consultations DESC) AS AgeGroup_Rank
FROM CardioConsultations;
...
TopFrequencies AS (
    SELECT DISTINCT Prescription_Count
    FROM MedicationFrequency
    ORDER BY Prescription_Count DESC
    LIMIT 3
)
SELECT mf.medication_id, mf.medication_name, mf.Prescription_Count
FROM MedicationFrequency mf
JOIN TopFrequencies tf ON mf.Prescription_Count = tf.Prescription_Count
ORDER BY mf.Prescription_Count DESC;

WITH HospitalDoctorCount AS (
    SELECT
        h.hospital_id,
        h.hospital_name,
...
MinDoctorCount AS (
    SELECT MIN(Doctor_Count) AS Min_Count
    FROM HospitalDoctorCount
)
SELECT hdc.hospital_id, hdc.hospital_name, hdc.Doctor_Count
FROM HospitalDoctorCount hdc
JOIN MinDoctorCount mdc ON hdc.Doctor_Count = mdc.Min_Count;

WITH CardiologyRooms AS (
    SELECT
        h.hospital_id,
        h.hospital_name,
        d.department_id,
    ...
    WHERE d.dep_name LIKE '%Cardiology%'
    GROUP BY h.hospital_id, h.hospital_name, d.department_id, d.dep_name
),
MaxRooms AS (
    SELECT hospital_id, MAX(Room_Count) AS Max_Rooms
    FROM CardiologyRooms
    GROUP BY hospital_id
)
SELECT cr.hospital_id, cr.hospital_name, cr.department_id, cr.department_name, cr.Room_Count
FROM CardiologyRooms cr
JOIN MaxRooms mr ON cr.hospital_id = mr.hospital_id AND cr.Room_Count = mr.Max_Rooms
ORDER BY cr.hospital_name;
...
SELECT
    ...
    DATEDIFF(appointment_date, Previous_Appointment) AS Days_Between
FROM PatientAppointments
WHERE Previous_Appointment IS NOT NULL
ORDER BY patients_name, appointment_date;


WITH SpecialistCount AS (
    -- Step 1: Count how many specialists each hospital has
    SELECT
        h.hospital_id,
        h.hospital_name,
        d.specialty,
        COUNT(d.doctor_id) AS Total_Specialists
    FROM hospitals h
    JOIN doctors d ON h.hospital_id = d.hospital_id
    WHERE d.specialty = 'Cardiology'   -- 🔹 Change this to any specialty
    GROUP BY h.hospital_id, h.hospital_name, d.specialty
),
RankedHospitals AS (
    -- Step 2: Rank hospitals from highest to lowest count
    SELECT
        hospital_id,
        hospital_name,
        specialty,
        Total_Specialists,
        RANK() OVER (ORDER BY Total_Specialists DESC) AS Specialist_Rank
    FROM SpecialistCount
)
-- Step 3: Return only hospitals with rank = 1 (maximum specialists)
SELECT
    hospital_id,
    hospital_name,
    specialty,
...
WHERE Specialist_Rank = 1;

WITH DeptRoomCapacity AS (
    SELECT
        d.department_id,
        d.dep_name AS department_name,
    ...
    GROUP BY d.department_id, d.dep_name
)
SELECT *
FROM DeptRoomCapacity
ORDER BY Avg_Capacity DESC;

WITH HospitalRoomCapacity AS (
    SELECT
        h.hospital_id,
        h.hospital_name,
        AVG(r.capacity) AS Avg_Capacity
...
WITH WeeklyTrend AS (
    SELECT
        YEAR(appointment_date) AS Year_,
...
WITH ReasonCount AS (
    SELECT
        reason,
        COUNT(appointment_id) AS Reason_Frequency
    FROM appointments
    GROUP BY reason
)
SELECT *
